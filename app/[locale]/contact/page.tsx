...
import { useEffect, useRef, useState, useCallback, MouseEvent, ReactNode } from "react";
...
function BudgetPills() {
  ...
  const opts = ["sub 3.000 lei","3.000 – 8.000 lei","8.000 – 20.000 lei","20.000 lei +","Discutăm"];
  return (
    <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
      {opts.map(o => (
        <button key={o} type="button" onClick={() => setSel(o)}
          style={{ padding:"8px 16px", borderRadius:999, border:`1px solid ${sel===o?"var(--ink)":"rgba(0,0,0,0.1)"}`, backgroundColor:sel===o?"var(--ink)":"transparent", color:sel===o?"#fff":"var(--muted)", fontSize:12, fontWeight:sel===o?600:400, fontFamily:"inherit", transition:"all .2s" }}>
          {o}
        </button>
      ))}
    </div>
...
function ContactForm() {
  ...
  if (sent) return (
    <div style={{ padding:"48px 0", textAlign:"center" }}>
      ...
      <h3 style={{ fontSize:20, fontWeight:700, color:"var(--ink)", margin:"0 0 10px", letterSpacing:"-0.02em" }}>Mesaj trimis.</h3>
      <p style={{ fontSize:14, fontWeight:300, color:"var(--muted)", margin:0, lineHeight:1.7 }}>Îți răspundem în maxim 24 de ore.</p>
    </div>
  );
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:20 }}>
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
        <div>
          <label style={{ fontSize:10, fontWeight:600, letterSpacing:"0.12em", textTransform:"uppercase", color:"var(--muted)", display:"block", marginBottom:8 }}>Nume</label>
          <input type="text" value={fields.name} onChange={set("name")} placeholder="Ion Popescu" onFocus={()=>setFocused("name")} onBlur={()=>setFocused(null)} style={inputStyle(focused==="name")}/>
        </div>
        <div>
          ...
          <input type="email" value={fields.email} onChange={set("email")} placeholder="ion@companie.ro" onFocus={()=>setFocused("email")} onBlur={()=>setFocused(null)} style={inputStyle(focused==="email")}/>
        </div>
      </div>

      <div>
        <label style={{ fontSize:10, fontWeight:600, letterSpacing:"0.12em", textTransform:"uppercase", color:"var(--muted)", display:"block", marginBottom:8 }}>Tipul proiectului</label>
        <select value={fields.projectType} onChange={set("projectType")} onFocus={()=>setFocused("project")} onBlur={()=>setFocused(null)}
          ...
          <option value="brand">Brand & Identitate Vizuală</option>
          <option value="ux">Design UX/UI</option>
          <option value="web">Dezvoltare Web</option>
          <option value="produs">Produs Digital</option>
          <option value="strategie">Strategie Digitală</option>
          <option value="altul">Altul</option>
        </select>
      </div>

      <div>
        <label style={{ fontSize:10, fontWeight:600, letterSpacing:"0.12em", textTransform:"uppercase", color:"var(--muted)", display:"block", marginBottom:10 }}>Investiție estimată</label>
        <BudgetPills/>
      </div>

      <div>
        <label style={{ fontSize:10, fontWeight:600, letterSpacing:"0.12em", textTransform:"uppercase", color:"var(--muted)", display:"block", marginBottom:8 }}>Mesaj</label>
        <textarea value={fields.message} onChange={set("message")} placeholder="Spune-ne despre proiectul tău, obiective, timeline..." rows={5} onFocus={()=>setFocused("message")} onBlur={()=>setFocused(null)} style={{ ...inputStyle(focused==="message"), resize:"vertical", minHeight:120 }}/>
      </div>
